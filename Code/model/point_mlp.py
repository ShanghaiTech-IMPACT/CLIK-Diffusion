...
class PointMLP(nn.Module):
    def __init__(self, num_classes=3,points=4096, embed_dim=128, groups=1, res_expansion=1.0,
        ...
        self.cls_map = nn.Sequential(
            ConvBNReLU1D(64, cls_dim, bias=bias, activation=activation),
        ...
        self.id_embedding = torch.nn.Embedding(32, 64)

        # classifier
        self.classifier = nn.Sequential(
            nn.Conv1d(gmp_dim+de_dims[-1]+cls_dim, 128, 1, bias=bias),
            nn.BatchNorm1d(128),
            nn.Dropout(),
    ...
    def forward(self, x, norm_plt, ids=None):
        xyz = x.permute(0, 2, 1) # x: [B, 3, N]
        ...
        id_token = self.cls_map(self.id_embedding(ids).unsqueeze(dim=-1))  # [b, 64, 1]
        x = torch.cat([x, global_context.repeat([1, 1, x.shape[-1]]), id_token.repeat(1, 1, x.shape[-1])], dim=1)
        x = self.classifier(x)
        x = x.permute(0, 2, 1)
        return x


def pointMLPNet(num_classes=3, **kwargs) -> PointMLP:
    return PointMLP(num_classes=num_classes, points=kwargs['points'], embed_dim=kwargs['embed_dim'], groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                 k_neighbors=[32, 32, 32, 32], reducers=[4, 4, 4, 4],
...
if __name__ == '__main__':
    ...
    ids = torch.randint(0, 32, (2,)).cuda()
    print(ids.shape)
    print("===> testing modelD ...")
    model = PointMLP(50).cuda()
    out = model(data, norm, ids)  # [2,2048,50]
    print(out.shape)